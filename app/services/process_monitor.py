...
SERVER_STARTING_GRACE_SECONDS = 90.0
...
class ProcessMonitor:
    def __init__(self, config: AppConfig, paths: AppPaths) -> None:
        self.config = config
        self.paths = paths
        self.state_dir = paths.user("state")
        ...
        self._server_was_online = False
        self._fxserver_running_since: float | None = None
    ...
    def server_status(self) -> ComponentStatus:
        ...
        if self._tcp_port_open(host, port):
            self._server_was_online = True
            return ComponentStatus("Server", "Online", f"Listening on {host}:{port}")

        fxserver = self.fxserver_status()
        state = self._bot_last_known_online()
        if state is False:
            return ComponentStatus("Server", "Offline", "No server port listener; last observed offline by Discord bot")

        if self._server_was_online:
            return ComponentStatus("Server", "Offline", f"Server port closed on {host}:{port}")

        if fxserver.state == "Running":
            running_seconds = self._fxserver_running_seconds()
            if running_seconds < SERVER_STARTING_GRACE_SECONDS:
                return ComponentStatus(
                    "Server",
                    "Starting",
                    f"FXServer running; server port not listening on {host}:{port}",
                )
            return ComponentStatus("Server", "Offline", f"FXServer shell is open; server port is closed on {host}:{port}")

        self._fxserver_running_since = None
        return ComponentStatus("Server", "Offline", f"Not listening on {host}:{port}")
    ...
    def server_is_offline(self) -> bool:
        return self.server_status().state == "Offline"
    ...
    def _server_endpoint(self) -> tuple[str, int]:
        ...
        match = re.search(r'endpoint_add_tcp\s+"([^"]+)"', cfg)
        if not match:
            return "127.0.0.1", 30120
    ...
    def _fxserver_running_seconds(self) -> float:
        if self._fxserver_running_since is None:
            self._fxserver_running_since = monotonic()
            return 0.0
        return monotonic() - self._fxserver_running_since
