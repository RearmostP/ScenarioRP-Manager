...
$DefaultManagerDir = Split-Path -Parent $AppDir
$ManagerDir = if ($env:SCENARIORP_MANAGER_ROOT) { $env:SCENARIORP_MANAGER_ROOT } else { $DefaultManagerDir }
$ProjectRoot = if ($env:SCENARIORP_MANAGER_PROJECT_ROOT) { $env:SCENARIORP_MANAGER_PROJECT_ROOT } else { Split-Path -Parent $ManagerDir }
$SystemDataDir = if ($env:SCENARIORP_MANAGER_SYSTEM_DATA) { $env:SCENARIORP_MANAGER_SYSTEM_DATA } else { Join-Path $ManagerDir "system_data" }
$UserDataDir = if ($env:SCENARIORP_MANAGER_USER_DATA) { $env:SCENARIORP_MANAGER_USER_DATA } else { Join-Path $ManagerDir "user_data" }
$ConfigPath = Join-Path $SystemDataDir "config.json"
...
function Get-ServerEndpoint {
    ...
    $line = Get-Content -LiteralPath $ServerCfg -ErrorAction SilentlyContinue |
        Where-Object { $_ -match 'endpoint_add_tcp' } |
    ...
    $separatorIndex = $endpoint.LastIndexOf(":")
    ...
    $hostName = $endpoint.Substring(0, $separatorIndex).Trim("[]")
    $portText = $endpoint.Substring($separatorIndex + 1)
